...
if ( ! class_exists( 'AIAmigos_Remediation_Plugin', false ) ) {
	final class AIAmigos_Remediation_Plugin {
		...
		public static function boot() {
			...
			add_filter( 'rank_math/opengraph/twitter/twitter_label1', array( __CLASS__, 'filter_unattested_twitter_author_label' ), 99 );
			add_filter( 'rank_math/opengraph/twitter/twitter_data1', array( __CLASS__, 'filter_unattested_twitter_author_value' ), 99 );
			...
			add_filter( 'option_users_can_register', array( __CLASS__, 'force_users_cannot_register' ), 99 );
			add_filter( 'pre_option_users_can_register', array( __CLASS__, 'force_users_cannot_register' ), 99 );
			...
			// malformed token; the bounded email fallback owns the request path.
		...
		/**
		 * Force public account registration off by default.
		 *
		 * Keeping this on in production-grade remediation removes anonymous self-service
		 * account creation, which is a high-risk surface for unmoderated contribution.
		 *
		 * @param mixed $users_can_register Option value from WP.
		 * @return bool
		 */
		public static function force_users_cannot_register( $users_can_register ) {
			return (bool) apply_filters( 'aiamigos_remediation_users_can_register', false, (bool) $users_can_register );
		}
		...
		/**
		 * Exact public demo, unsupported, empty, child-risk, affiliate, and
		 * unfulfilled-artifact paths that must return a reversible 410 tombstone.
		 *
		 * @return array
		 */
		...
		/**
		 * Seeded demo, retired service, unsupported, empty, child-risk, affiliate,
		 * and unfulfilled-artifact records retained only for recoverable admin access.
		 *
		...
		private static function quarantined_post_ids() {
			$ids = array( 34, 36, 46, 48, 50, 40, 42, 44, 26, 28, 30, 32, 945, 441, 783, 1405, 552, 6, 1411, 1407, 1421, 926, 21, 22, 877, 149, 157, 1417 );
		...
		private static function exact_description_override() {
			...
			$descriptions = array(
				...
				'member-name-01'   => 'This profile is retained while identity, role, authorship, and editorial responsibility await accountable review.',
				'about-us'         => 'AI Amigos publishes educational articles about artificial intelligence. Authorship, reviewers, and qualifications remain under review.',
		...
		/**
		 * Fail closed on unapproved entity types and normalize same-site HTTP
		 * references. Only structural WebSite, WebPage, BreadcrumbList, ListItem,
		 * and ImageObject nodes remain until accountable evidence packets approve
		 * authors, publishers, articles, products, organizations, or other claims.
		 *
		...
		public static function filter_unverified_schema( $data, $jsonld = null ) {
			...
			$before_ids = array();
			self::collect_schema_ids( $data, $before_ids );
			$data = self::retain_approved_schema_types( $data );
			$after_ids = array();
			self::collect_schema_ids( $data, $after_ids );
			$removed_ids = array_diff_key( $before_ids, $after_ids );
			$data = self::remove_unverified_schema_relationships( $data, $removed_ids );
			return self::normalize_same_site_schema_urls( $data );
		}

		/**
		 * Remove Rank Math's slot-one label on standard single posts, where the
		 * captured contract uses that slot for an unapproved author identity.
		 * Page reading-time and archive-count uses remain available.
		 *
		 * @param mixed $content Existing Twitter label value.
		 * @return mixed
		 */
		public static function filter_unattested_twitter_author_label( $content ) {
			return is_singular( 'post' ) ? false : $content;
		}

		/**
		 * Remove the paired slot-one value on standard single posts. This prevents
		 * both generic and named authors from being asserted before the 71-post
		 * responsibility ledger is approved. Preserve page/archive uses.
		 *
		 * @param mixed $content Existing Twitter data value.
		 * @return mixed
		 */
		public static function filter_unattested_twitter_author_value( $content ) {
			return is_singular( 'post' ) ? false : $content;
		}

		/**
		 * Structural schema types that do not assert an accountable real-world
		 * author, publisher, product, service, course, or article identity.
		 *
		 * @return array<int,string>
		 */
		private static function approved_schema_types() {
			return array( 'website', 'webpage', 'breadcrumblist', 'listitem', 'imageobject' );
		}

		/**
		 * Collect every typed-definition identifier in a schema subtree. Bare
		 * @id-only references are deliberately excluded: counting them as surviving
		 * definitions would conceal a dangling reference after its definition is
		 * swallowed inside a rejected parent. Diffing before and after type pruning
		 * therefore captures omitted descendants without rejecting an ID whose
		 * typed structural definition genuinely survives elsewhere.
		 *
		 * @param mixed              $value       Schema subtree.
		 * @param array<string,bool> $ids Identifiers, keyed by ID.
		 * @return void
		 */
		private static function collect_schema_ids( $value, &$ids ) {
			if ( ! is_array( $value ) ) {
				return;
			}

			if ( isset( $value['@type'], $value['@id'] ) && is_string( $value['@id'] ) && '' !== $value['@id'] ) {
				$ids[ $value['@id'] ] = true;
			}

			foreach ( $value as $child ) {
				self::collect_schema_ids( $child, $ids );
			}
		}

		/**
		 * Recursively retain only structural schema types that do not assert a
		 * real-world author, publisher, product, service, or article identity.
		 *
		 * @param mixed $value Schema subtree.
		 * @return mixed|null
		 */
		private static function retain_approved_schema_types( $value ) {
			if ( ! is_array( $value ) ) {
				return $value;
			}
			...
			if ( isset( $value['@type'] ) ) {
				...
				$allowed = self::approved_schema_types();
			...
			foreach ( $value as $key => $child ) {
				$filtered = self::retain_approved_schema_types( $child );
				if ( null === $filtered ) {
					unset( $value[ $key ] );
				} else {
					$value[ $key ] = $filtered;
		...
		/**
		 * Remove identity-bearing relationships and references to every omitted
		 * entity. Untyped inline author/creator/publisher objects are removed by
		 * their relationship key; references under other keys are removed by ID.
		 *
		 * @param mixed              $value       Schema subtree.
		 * @param array<string,bool> $removed_ids Removed identifiers, keyed by ID.
		 * @return mixed|null
		 */
		private static function remove_unverified_schema_relationships( $value, $removed_ids = array() ) {
			if ( is_string( $value ) && isset( $removed_ids[ $value ] ) ) {
				return null;
			}
			if ( ! is_array( $value ) ) {
				return $value;
			}
			if ( isset( $value['@id'] ) && is_string( $value['@id'] ) && isset( $removed_ids[ $value['@id'] ] ) ) {
				return null;
			}

			$is_list = empty( $value ) || array_keys( $value ) === range( 0, count( $value ) - 1 );
			$identity_relationships = array(
				'about', 'accountablePerson', 'affiliation', 'agent', 'alumniOf',
				'author', 'brand', 'broker', 'contributor', 'copyrightHolder',
				'creator', 'customer', 'editor', 'employee', 'founder', 'funder', 'knows',
				'mainEntity', 'mainEntityOfPage', 'manufacturer', 'member',
				'maintainer', 'organizer', 'owner', 'parentOrganization', 'performer', 'producer',
				'provider', 'publisher', 'reviewedBy', 'seller', 'sponsor',
				'publisherImprint', 'sdPublisher', 'sourceOrganization',
				'subOrganization', 'subjectOf', 'translator', 'worksFor',
			);
			foreach ( $identity_relationships as $relationship ) {
				unset( $value[ $relationship ] );
			}
			foreach ( $value as $key => $child ) {
				$filtered = self::remove_unverified_schema_relationships( $child, $removed_ids );
				if ( null === $filtered ) {
					unset( $value[ $key ] );
				} else {
					$value[ $key ] = $filtered;
		...
		public static function render_bounded_institutional_page( $content ) {
			...
			if ( 'about-us' === $slug ) {
				...
				return sprintf(
					...
					esc_html__( 'Archived article bylines and reviewer identities remain under review. AI Amigos does not currently make a sitewide authorship, reviewer, or professional-qualification claim.', 'aiamigos-remediation' ),
		...
		/**
		 * Replace broken MailPoet IDs 2/3 with an honestly labelled email request.
		 *
		...
		/**
		 * Render publication dates without asserting an unreviewed author identity.
		 *
		...
		public static function prepend_accountable_byline( $content ) {
			...
			$byline = sprintf(
				'<p class="aiamigos-accountable-byline"><span>%1$s <time datetime="%2$s">%3$s</time></span>',
