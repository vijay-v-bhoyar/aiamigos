...
- Filters Rank Math JSON-LD fail-closed on the homepage: unapproved Article, Person, Organization, and EducationalOrganization nodes plus their relationships are removed while WebSite/WebPage nodes remain. HTTP URLs on the exact AI Amigos host aliases are upgraded to HTTPS throughout Rank Math schema; unrelated external hosts are unchanged.
...
- Replaces MailPoet form IDs 2/3 with an honest `mailto:contact@aiamigos.org` update-request link while privacy and retention review remains open. Contact Form 7 scripts/styles are dequeued sitewide by default. `aiamigos_remediation_enable_collection_form` may release the single exact published CF7 form titled `Subscribe Newsletter Form` only in the main `/newsletter/` content after an approved data-flow record; every other location and any empty, missing, duplicate, non-published or ambiguous match still fails closed to email.
...
5. `/newsletter-2/` redirects to `/newsletter/`; only the main `/newsletter/` content can render CF7, and other pages carry no CF7 scripts/styles. The form is an update request and does not promise subscription.
6. All eight exact placeholder routes and four exact retired `/services/` routes return `410`; IDs `34, 36, 46, 48, 50, 40, 42, 44, 26, 28, 30, 32` are absent from public listings/search/sitemaps. Team ID `38` and `/team/member-name-01/` remain available.
...
16. Single posts expose an accountable author/published line, and an updated date only where warranted.
...
- `aiamigos_remediation_enable_collection_form`
...
On 2026-08-13, a hash-verified official PHP 8.4.24 NTS CLI passed `php -l` for all 16 local plugin/release/theme artifacts, and `tests/test-policy.php` passed 28/28 assertions. Re-run both gates after every PHP edit and again in staging/CI; local CLI proof does not establish Hostinger web-SAPI compatibility.
