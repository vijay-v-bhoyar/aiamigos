...
5. `synchronizeCarousel()` - 11 edges
6. `wireSearchControls()` - 11 edges
7. `wireMenuControls()` - 9 edges
8. `is_singular()` - 9 edges
9. `FakeClassList` - 7 edges
10. `wp_unslash()` - 7 edges
...
## Knowledge Gaps
- **44 isolated node(s):** `failures`, `testRoot`, `tests`, `failures`, `files` (+39 more)
...
- **Are the 2 inferred relationships involving `synchronizeCarousel()` (e.g. with `titleFromSlide()` and `.hidden()`) actually correct?**
  _`synchronizeCarousel()` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `failures`, `testRoot`, `tests` to the rest of the system?**
